...
<?php
	$payroll=$conn->query("SELECT p.*,concat(e.lastname,', ',e.firstname,' ',e.middlename) as ename,e.employee_no FROM payroll_items p inner join employees e on e.employeeid = p.employee_id  where p.id=".$_GET['id']);
	foreach ($payroll->fetch_array() as $key => $value) {
		$$key = $value;
	}
	$pay = $conn->query("SELECT * FROM payroll where payrollid = ".$payroll_id)->fetch_array();
...
<div class="contriner-fluid">
	<div class="col-md-12">
		...
		<div class="row">
			<div class="col-md-6">
				<p><b>Payroll Ref: </b> <?php echo $pay['ref_no'] ?></p>
				<p><b>Payroll Range : </b><?php echo date("M d, Y",strtotime($pay['date_from'])). " - ".date("M d, Y",strtotime($pay['date_to'])) ?></p>
				<p><b>Payroll type : </b> <?php echo $pt[$pay['type']] ?></p>
			</div>
			<div class="col-md-6">
				<p><b>Days of Absent :</b> <?php echo $absent ?></p>
				<p><b>Tardy/Undertime (mins) : </b><?php echo $late ?></b></p>
				<p><b>Total Allowance Amount : </b><?php echo number_format($allowance_amount,2) ?></p>
				<p><b>Total Deduction Amount : </b><?php echo number_format($deduction_amount,2) ?></p>
				<p><b>Net Pay : </b><?php echo number_format($net,2) ?></p>
			</div>
		</div>
					...
					<div class="card-body">
						<ul class="list-group">
							...
							while($row=$all_qry->fetch_assoc()):
								$all_arr[$row['allowanceid']] = $row['allowance'];
						 	endwhile; 
				...
				<div class="card">
					...
					<div class="card-body">
						<ul class="list-group">
							...
							$t_arr = array(1=>"Monthly",2=>"Semi-Monthly",3=>"Once");
							while($row=$all_qry->fetch_assoc()):
								$ded_arr[$row['deductionid']] = $row['deduction'];
						 	endwhile; 

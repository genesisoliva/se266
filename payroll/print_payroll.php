...
<?php
		$pay = $conn->query("SELECT * FROM payroll where payrollid = ".$_GET['id'])->fetch_array();
...
<table>
    ...
    <?php
        ...
        $payroll = $conn->query("SELECT p.*,concat(e.lastname,', ',e.firstname,' ',e.middlename) as ename,e.employee_no,e.salary FROM payroll_items p inner join employees e on e.employeeid = p.employee_id ") or die(mysqli_error());

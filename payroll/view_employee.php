...
<?php
$emp = $conn->query("SELECT e.*, d.name as dname, p.name as pname FROM employees e inner join departments d on e.department_id = d.departmentid inner join positions p on e.position_id = p.positionid where e.employeeid =".$_GET['id'])->fetch_array();
	foreach($emp as $k=>$v)
	{
		$$k=$v;
	}
...
<div class="contriner-fluid">
	<div class="col-md-12">
		<h5><b><small>Employee #: </small><?php echo $employee_no ?></b></h5>
		<h4><b><small>Name: </small><?php echo ucwords($lastname.", ".$firstname." ",$middlename) ?></b></h4>
		<p>Department: <b><?php echo ucwords($dname) ?></b></p>
		<p>Position: <b><?php echo ucwords($pname) ?></b></p>
			...
			<div class="col-md-6">
				<div class="card">
					<div class="card-header">
						...
						<button class="btn btn-primary btn-sm float-right" style="padding: 3px 5px" type="button" id="new_allowance"><i class="fa fa-plus"></i> Add</button>
					</div>
					<div class="card-body">
						<ul class="list-group">
							<?php
							$allowances = $conn->query("SELECT ea.*,a.allowance as aname FROM employee_allowances ea inner join allowances a on a.allowanceid = ea.allowance_id where ea.employee_id=".$_GET['id']." order by ea.type asc,date(ea.effective_date) asc, a.allowance asc ");
			...
			<div class="col-md-6">
				<div class="card">
					<div class="card-header">
						...
						<button class="btn btn-primary btn-sm float-right" style="padding: 3px 5px" type="button" id="new_deduction"><i class="fa fa-plus"></i> Add</button>
					</div>
					<div class="card-body">
						<ul class="list-group">
							<?php
							$deductions = $conn->query("SELECT ea.*,d.deduction as dname FROM employee_deductions ea inner join deductions d on d.deductionid = ea.deduction_id where ea.employee_id=".$_GET['id']." order by ea.type asc,date(ea.effective_date) asc, d.deduction asc ");
							$t_arr = array(1=>"Monthly",2=>"Semi-Monthly",3=>"Once");
